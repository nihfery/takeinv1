...
@php
    $branches = collect([
        (object)['id' => 1, 'branch_name' => 'Aura Studio Central'],
        (object)['id' => 2, 'branch_name' => 'Aura Studio West'],
    ]);

    $categories = collect([
        (object)['id' => 1, 'name' => 'Hair Stylist'],
        (object)['id' => 2, 'name' => 'Barber'],
    ]);
    ...
    $staffs = collect([
        (object)[
            ...
            'providerRole' => (object)['role_name' => 'Senior Stylist'],
            'created_at' => now(),
        ],
        (object)[
            'id' => 2,
            ...
            'providerRole' => (object)['role_name' => 'Staff'],
            'created_at' => now(),
        ]
    ]);
...
<img src="data:image/svg+xml;utf8,<svg width='32' height='32' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'><path d='M5.5 3L18.5 13L12.5 14L15.5 21L12.5 22L9.5 15L4.5 17V3Z' fill='white' stroke='%23111827' stroke-width='1.5' stroke-linejoin='round'/></svg>" id="demo-cursor" alt="Cursor" />

<style>
    /* Simulator Cursor Styles */
    #demo-cursor {
        position: fixed;
        top: 50%;
        left: 50%;
...
<style>
    /* Stretch the white card so there's no gray gap at the bottom */
    .admin-booking-card {
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        setTimeout(() => {
            // Find staff row in desktop table
            const rows = document.querySelectorAll('.provider-staff-category-table tbody tr');
            // Find staff card in mobile view
            const cards = document.querySelectorAll('.admin-category-mobile-card');
            
            const staff1 = rows.length > 0 ? rows[0] : (cards.length > 0 ? cards[0] : null);
            const addBtn = document.querySelector('[data-staff-add]');
            const filterBtn = document.querySelector('button[type="submit"]'); // The Filter button
            
            if (!staff1) return; // safeguard
            
            // The edit button in table or mobile
            const editBtn = staff1.querySelector('.staff-edit-btn');
            ...
            const moveCursorTo = (element, offsetX = 0, offsetY = 0) => {
                ...
                return new Promise(resolve => {
                    ...
                    currentX = rect.left + (rect.width / 2) + offsetX;
                    currentY = rect.top + (rect.height / 2) + offsetY;
                    cursor.style.transform = `translate(${currentX}px, ${currentY}px)`;
            ...
            async function runSimulation() {
                while (true) {
                    if (staff1) staff1.classList.remove('demo-hover-effect');
                    await wait(1000);

                    // Move to staff item
                    if (staff1) {
                        await moveCursorTo(staff1);
                        staff1.classList.add('demo-hover-effect');
                        await wait(600);
                    }

                    // Move to edit button
                    if (editBtn) {
                        await moveCursorTo(editBtn);
                        await wait(300);
                        await clickCursor();
                    ...
                    // Move to Add Staff button
                    if (addBtn) {
                        await moveCursorTo(addBtn);
                        await wait(300);
                        await clickCursor();
